...
// ---------------------------------------------------------------------------
// Mock the native global fetch before importing OpenAIProvider.
...
beforeEach(() => {
  mockFetch.mockReset();
  provider = new OpenAIProvider(makeApiKey(), makeModel());
});
...
describe('OpenAIProvider.analyzeOutdated()', () => {
  ...
  it('calls the OpenAI API endpoint with POST method', async () => {
  ...
  it('sends Authorization header with the provided API key', async () => {
    ...
    const p = new OpenAIProvider(apiKey, makeModel());
    mockOkResponse(wrapContent({
      summary: 'ok',
      priorityPackage: 'pkg',
      upgradeAdvice: 'upgrade',
  ...
  it('includes only status and statusText when response body is not JSON', async () => {
    ...
    await expect(provider.analyzeOutdated([])).rejects.toThrow('OpenAI API error: 502 Bad Gateway');
  });
});
...
describe('OpenAIProvider.analyzeBundleSize()', () => {
...
describe('OpenAIProvider.analyzeLicenses()', () => {
...
describe('OpenAIProvider.analyzeUnused()', () => {
